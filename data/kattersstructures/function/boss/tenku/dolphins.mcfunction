...
execute as @e[type=turtle,tag=katter.tenku.vehicle] at @s unless entity @e[type=dolphin,tag=katter.tenku.asset,distance=..16] unless score @s kattersstructures.tenku matches ..70 run function kattersstructures:boss/tenku/spawn_assets